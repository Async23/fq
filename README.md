...
选择器采用接近 btop 进程面板的操作方式：方向键浏览，`f` 或 `/` 进入筛选，`t` 正常退出，`k` 强制退出，`q` 关闭。也可以直接输入非命令字符开始模糊筛选；匹配范围包括应用名、Bundle ID 和 PID。

`Enter` 执行标题栏所示的当前模式（默认是强制退出，`--quit` 时是正常退出）。`Home` / `End` 和 `PgUp` / `PgDn` 可快速移动，`Delete` 或 `Ctrl-U` 清空筛选，`?` 打开选择器内帮助。

强制退出的确认会留在全屏选择器内，并且必须明确输入 `y`；`Enter` 不会确认破坏性操作。按 `n` 或 `Esc` 返回应用列表。Finder 会显示专门的“将由 macOS 自动重新打开”提示。
